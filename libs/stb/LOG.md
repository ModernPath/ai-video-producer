...
**Deferred:** REQ-STB-064 (export-time offset for uploaded tracks) unchanged.
**Discovered:** (a) **Section alignment needs the OMNI route, and ADR-013 does not say so.** Veo's durations are {4,6,8} — all even — so every reachable cumulative boundary is even and an odd stamp like 0:23 or 0:47 is arithmetically unreachable; omni's 4–10 integers reach both. Computed, not guessed. Found by READING the golden file, which printed `"durationS":4|6|8` — precisely the argument for golden files over `toContain`. The ADR stays immutable; this is recorded here and in the ledger, and if it should live in the record it needs a superseding ADR. (b) Two of my new integration-test inserts passed `organizationId` to `script_version` and `music_brief`, neither of which has that column. **The tests passed** — drizzle ignored the unknown key — and only `pnpm typecheck` caught it, four hours after REQ-GEN-033 repaired that gate from 45 errors to 0. It has now paid for itself.
**Follow-ups:** decide whether the omni-route constraint warrants ADR-014; human sign-off on this row.
**Gate:** typecheck 0 (libs + web); **531 passed / 14 skipped**; web builds; service gate mutation-verified (removing the throw turns both refusal tests red); **LOOKED AT** in the running product on a real `lyric-video` project — blocker sentence rendered, button disabled, probe project cleaned up afterwards.

## 2026-07-28 — OQ-115 resolved by ADR-013: music-led films plan against the real track (REQ-STB-032 BLOCKED → READY)
**Done:** ADR-013 written and accepted; OQ-115 closed; REQ-STB-032 restated and unblocked; REQ-STB-064 opened for the one surviving fragment of the old plan. No code yet — this is the decision made reviewable before anything is built, per §4B.
**Decisions:** USER:2026-07-28 rejected the framing of my own recommendation: "the only way to nail it is to get the actual transcript and timestamps of the music BEFORE we generate any videoclips". They are right. OQ-115 offered (a) pad the pictures to reach the music and (b) trim the music to reach the pictures — **both downstream repairs of an ORDERING problem**, and I had recommended (c) both, building (b) first. For music-led archetypes the shot plan now waits for a transcribed track and receives its `[MM:SS]` stamps. Two facts settled it, and neither was in the OQ: (1) the chain is already acyclic in the needed direction — brief ← script, track ← brief, transcript ← track, and `proposeShotPlan` ALREADY reads the music brief (REQ-STB-028), so only the track is missing at planning time; (2) the cost ratio is ~75:1 the wrong way round — `musicPerTrackUsd` 0.08 against `videoPerSecondUsd` 0.10, so a 60s film is $6.00 of video informed by an $0.08 artifact we currently generate LAST. Option (a) would have bought filler footage at $0.10/s to cover intros, ~$2.30 per render on the Neon Rivers track, every render.
**Deferred:** REQ-STB-064 (export-time offset) — survives only for the Suno path, where a finished track arrives after the plan and nothing can be reordered.
**Discovered:** (a) **the alignment floor is arithmetic, not taste** — `shot.minSeconds` is 4 and lyric lines are commonly 1–2s apart, so per-line shot alignment is impossible on any route; section-level is the honest ceiling and captions already own line-level precision. This is recorded in the ADR so nobody re-litigates it. (b) I had recommended (c) an hour before this; the user's question exposed that I was optimising a repair instead of asking why the repair existed. Worth naming: **when two options both look like corrections, the defect is usually upstream of both.**
**Follow-ups:** build REQ-STB-032 — the gate reuses `generationBlocker`, and the plan prompt gains a golden file per §1.9.
**Gate:** docs + ledger only, no code touched. ADR-013 indexed; OQ-115 moved to the resolved list with its resolution recorded; ledger totals reconciled (1 BLOCKED → 0, REQ-STB-032 READY).

## 2026-07-27 — REQ-STB-061 all three escapes covered (IN_PROGRESS → DONE) — CORRECTS this morning's entry
**Done:** REQ-STB-045 now has a real render test, mutation-verified. Remove `key={props.shot.id}` from StagePanel's root and it fails reporting `Received: "MY UNSAVED EDIT"` — the user's exact symptom, the previous shot's typed text appearing in the next shot's box. All 3 escapes (045/057/058) covered; `apps/web` is at 20 tests across 4 files, from 1 at the start of the day.
**Decisions:** (1) Environment is **jsdom**, not happy-dom, for this spec: jsdom implements the DOM dirty-value flag, so an uncontrolled input behaves as it does in a browser. (2) **No Playwright.** The earlier entry said faithful coverage needed a browser tier; that was wrong, and no new dependency tier was required. (3) `stage-panel-identity.spec.tsx` corrected — it guarded the OUTER key in page.tsx, which is redundant; the panel root's key is the mechanism. Both are asserted now, the inner one labelled as load-bearing.
**Deferred:** none for this row.
**Discovered — this entry CORRECTS the one dated earlier today.** That entry stated REQ-STB-045 "cannot be covered in this harness" and blamed happy-dom's missing dirty-value flag plus RTL's `rerender`. The real cause was my own mutation being aimed at the wrong place: I removed the key from `<StagePanel>` in page.tsx, but when the panel was extracted the key came WITH it onto its root `<div>`, so the remount happened regardless and the test passed either way. I read "passes with the fix removed" as "the harness cannot see this", when it actually meant "you removed the wrong fix". Two lessons, both cheap and both mine: **a negative mutation result is only evidence if you have confirmed the mutation actually disables the behaviour** — check the mutated code renders differently before concluding anything about the harness; and **when a refactor moves JSX, it moves the props ON that JSX**, so any guard pointing at the old location silently stops guarding.
**Follow-ups:** `StagePanel.tsx` is 525 lines, still over §10B — worth a PROPOSED row. No CI runs `pnpm check` (from the GEN-033 entry) — also worth one.
**Gate:** typecheck 0 (libs + web); **517 passed / 14 skipped**; each of the 3 regression tests verified red against its own mutation and green after restore.
...
**Deferred:** REQ-STB-045 render coverage — needs a real browser (Playwright). Recorded, not papered over.
**Discovered:** **the REQ-STB-045 test I wrote was vacuous, and only mutation testing found it.** Three successive versions passed with `key={shot.id}` REMOVED: first comparing displayed values, then comparing values after editing the box, then comparing node identity across the swap. Measured cause: happy-dom implements no DOM "dirty value flag", so an uncontrolled input appears to re-read its `defaultValue` even when React keeps the node — the precise thing that did NOT happen in the user's browser — and RTL's `rerender` produced a fresh element with and without the key. So this harness cannot reproduce reconciliation-reuse at all. That is the second time in this project I have written a guard test that asserted on its own setup rather than on the code (the first is in the GEN log); the difference is that this time the mutation check caught it before it shipped. **A test for a UI defect is worthless until you have watched it fail against the unfixed code** — worth adding to §6B.
**Follow-ups:** REQ-STB-045 browser-level coverage (PROPOSED-worthy); the 7 remaining panels of REQ-STB-060.
**Gate:** typecheck 0 (libs + web); 511 passed / 14 skipped; `apps/web` 14 tests across 3 files, up from 1 source-text spec.

## 2026-07-27 — REQ-STB-059 split service.ts by aggregate (READY → DONE)
**Done:** 1,147 lines / 40 runtime exports → ten modules, largest 211L. `service.ts` is a 14-line barrel so the 26 specs and `apps/web` that import it keep working **with no test changed** — the acceptance criterion, and the reason the barrel exists at all.
**Decisions:** (1) **Measured the seam before cutting it.** A script mapped every private helper to its callers: all 7 were cross-aggregate (`getShotOrThrow` used by 14 exports, `resolveCast` by 6, `assertDuration` by 2), so all 7 went to `common.ts` rather than being guessed into an aggregate. That is acceptance criterion 3 — helpers in a named module, not re-exported sideways. (2) `common.ts` exports its helpers to STB but only `StbValidationError` and `DirectionJson` reach the package surface; the barrel names those two explicitly instead of `export *`, so the internals cannot leak. (3) Cross-aggregate calls import the other module's PUBLIC export (`materialize` → `latestScript`, `plan` → `applyShotPlan` neighbours); the resulting graph is acyclic — common → shots/script → continuity/music → takes/plan/materialize.
**Deferred:** none.
**Discovered:** (a) the size test was genuinely RED first (`service.ts (1148)`) and named the only offender, which is what made it an acceptance criterion rather than a description. (b) The surface test was written GREEN on purpose as a regression harness — `export *` would have hidden a dropped export until a consumer crashed at runtime, so it also asserts every name is defined, not merely present. (c) First generation pass produced 62 type errors from two of my own bugs — `async function` missed the `export` prefix, and cross-aggregate calls were unaccounted for. Reverted and regenerated rather than patching the output, which came back at 0 errors.
**Follow-ups:** REQ-STB-060 (`p/[id]/page.tsx`) is the remaining §10B offender.
**Gate:** typecheck 0 errors; `libs/stb` 40 files / 222 tests green; full suite 504 passed with one `ast` derivative spec failing at load 7.8 and passing alone — the same environmental flake recorded before, not a regression. `apps/web` builds.
