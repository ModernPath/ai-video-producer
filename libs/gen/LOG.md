...
## 2026-07-28 — REQ-GEN-037 the video route is explicit wherever generation runs (→ IN_REVIEW)
**Done:** `GEN_VIDEO_ROUTE: omni` declared at `env.clear` in `config/deploy.yml`, so the web AND worker roles resolve the same route; `apps/worker` now loads the same `.env.local` the web app does, closing the same split locally; recent generations name their `modelId` beside the cost.
**Decisions:** (1) At `env.clear`, not per role — a per-role declaration would reinstate exactly the split this closes, and the guard test asserts no role block mentions the variable. (2) The route is surfaced in the product because the failure mode is SILENCE: a wrong route produces a working film with the wrong duration palette, which is only noticed after the fact.
**Deferred:** none.
**Discovered:** (a) **the repo's own guard caught me.** REQ-GEN-007 forbids model-id literals outside `libs/shared/src/config`, and my new render test hardcoded `gemini-omni-flash-preview`. It failed the unit ring immediately. Deriving it from `omniVideoModel` is also the better test — it now proves the panel shows whatever the ROUTE resolves to rather than a string I typed. That guard was written weeks ago and paid off today. (b) **§9.9 could not be satisfied.** Local dev now sits behind sign-in (REQ-PLT-002), and signing in is not something an agent should do on the user's behalf. Verified at the render layer instead — which is precisely what REQ-STB-061's harness exists for — and the ledger row records the gap honestly rather than implying a browser check happened. **The deployed fix stays unverified until the next deploy.**
**Follow-ups:** confirm on the deployed host after the next `kamal deploy` that a take records the omni model; sign-off.
**Gate:** typecheck 0 (libs + web); `pnpm test:unit` **413 passed across 40 files**; `deploy-config.spec.ts` mutation-verified — deleting the declared route turns 2 tests red.

## 2026-07-28 — REQ-GEN-036 a failed take says why (→ IN_REVIEW); REQ-GEN-037 raised
**Done:** USER BUG: "Generate the chain (10 shots)" → `take props.failed · output_unusable` / `no video in response: {}`. Three separate defects behind one screenshot. (1) `videoFailure()` — pure, 5 tests — reads the RESPONSE's `raiMediaFilteredCount`/`raiMediaFilteredReasons` and reports `content_policy` with the reason; a filtered generation SUCCEEDS with `op.error` empty, so the one field we printed was the one guaranteed to be empty in the likeliest case. (2) two more `props.` corruptions in rendered text, including the banner the user photographed. (3) REQ-GEN-037 raised: `GEN_VIDEO_ROUTE=omni` exists only in `apps/web/.env.local`; `config/deploy.yml` never declares it, so every deployed process runs the `"veo"` default.
**Decisions:** `content_policy`, not `output_unusable` — retrying the same prompt filters again, and the UI offers "Retry this take" right below the message. Route mis-configuration recorded as PROPOSED rather than fixed: declaring it changes deployed behaviour, which is the user's call.
**Deferred:** REQ-GEN-037.
**Discovered:** **my REQ-STB-067 text guard passed while the bug was on screen.** It asserts `container.textContent` carries no `props.`, which is the right assertion — but it renders ONE state, and a shot with no failed take never renders the failure banner. A guard that only visits the happy path only guards the happy path. Replaced with a six-state matrix (failed take, failed sub-clip, mid-generation, blocked chain member, custom prompts, plain), mutation-verified by restoring the exact corruption the user saw: 3 tests go red. This is the THIRD time this corruption class has surfaced — seven found by an HTML diff on 2026-07-27, two more by the user on the deployed app, two more now — all from one extraction that prefixed identifiers instead of destructuring. The lesson is not "grep harder": it is that **a text-corrupting refactor needs a test that walks the states, written before the refactor, not a grep after it.**
**Follow-ups:** REQ-GEN-037 needs the user's call; sign-off on REQ-GEN-036.
**Gate:** typecheck 0 (libs + web); `pnpm test:unit` 406 passed across 39 files; state-matrix guard mutation-verified.

## 2026-07-28 — REQ-GEN-035 CI runs the gate (PROPOSED → IN_REVIEW)
**Done:** `.github/workflows/ci.yml` runs `pnpm typecheck` then `pnpm test:unit` on every push and PR. Verified on the real runner, not locally: green in 31s, 33 spec files / 354 tests, `tsc -b` confirmed in the log. USER:2026-07-28 chose GitHub Actions and the narrow ring.
**Decisions:** (1) **Narrow on purpose.** The 62 `*.int.spec.ts` files need Postgres and MinIO and already flake at load ≥ 7 locally; a CI that is red for reasons that are not the code is worse than none, and would train everyone to ignore it. They stay local until that flakiness is understood. The real-API ring stays manual per §9.8 and is excluded by name AND by pattern. (2) `scripts/test-setup.ts` gained an `AVD_SKIP_DB_SETUP` guard rather than a second vitest config — one branch cannot drift from itself (§1.11). Proved it by running `pnpm test:unit` against a DEAD database host: 354 passed. (3) typecheck is a separate step from tests so a red build says "types" or "tests" in the job name, not just "check".
**Deferred:** widening CI to the integration ring — needs the `ast` derivative flakiness understood first.
**Discovered:** (a) **the gate is mutation-verified**, because a CI that cannot fail is theatre: a planted type error exits 2 and names the file; a planted failing spec exits 1. Both checked before pushing. (b) The first green run still carried a deprecation annotation — all three actions targeted Node 20 and were being forced onto 24. Green is not the same as clean; it took two more pushes (checkout/setup-node → v5, pnpm/action-setup → v6) to get an annotation-free run. Worth remembering that a tick mark hides warnings that become failures later.
**Follow-ups:** human sign-off; consider a branch-protection rule so a red check actually blocks merge — the workflow reports, but nothing yet enforces.
**Gate:** three consecutive green runs on `main` (30334593584, 30334662862, 30334732544); the last with zero annotations. Locally: typecheck 0, `pnpm test:unit` 354 passed, full suite 531 passed / 14 skipped.

## 2026-07-27 — REQ-GEN-033 lint + config hardening (READY → DONE) — the gate, not the rules
**Done:** repaired `pnpm typecheck` from **45 errors across 11 files to 0** and made it enforceable: new `pnpm check` (typecheck && test), and `CLAUDE.md` §9.3 now names that command instead of the unrunnable "architecture/lint checks pass". Guard spec `libs/shared/tests/type-gate.spec.ts` compiles fixtures and asserts the compiler REJECTS each of the three hazards, with positive controls; the vocabulary case lives in `libs/stb/tests/casting.spec.ts` (shared may not import stb).
**Decisions:** (1) **No linter added.** The row asked for `no-dupe-keys`; measured first, and TypeScript already rejects all three hazards — duplicate config key TS1117, whole `{bytes,mime}` assigned TS2740, passed to `Buffer.from` TS2769. Adding ESLint would have installed a fourth gate nobody runs beside three that already worked. The defect was never a missing rule; it was 45 errors of noise in front of a compiler that was right. (2) `migrate` moved to `@avd/shared/migrate` — 33 specs imported `../../../scripts/migrate`, reaching out of every lib's `rootDir` for 10 of the errors; migration is infrastructure and belongs in a lib. `scripts/migrate.ts` is now a CLI wrapper. (3) `configForTest` (`@avd/shared/config/testing`) is the sanctioned mutable view; production `config` stays deeply readonly, and 13 errors were specs assigning through it — specs that already restore correctly in `afterEach`. (4) `types: ["node"]` in `tsconfig.base.json`; `@types/node` was declared only in `apps/web`, which is why `process` and `console` were unresolved in libs *source*.
**Deferred:** none.
**Discovered:** (a) the three "hazards the type system did not catch" in `docs/88-architecture-review.md` §5 were mis-diagnosed **by me, in that review** — the type system caught all three; nothing ran it. A review finding that names a fix without measuring the mechanism can send the next session shopping for tooling it does not need. (b) 5 fake providers were missing `generateMusic` and 2 style-card fixtures were missing `continuity` — test doubles silently drifted behind contracts they claim to implement, invisible while the gate was red. (c) 3 specs previously called "environmental flakes" passed here at load 6.13, confirming that reading.
**Follow-ups:** no CI runs `pnpm check` — the gate is repaired but still depends on someone typing it. Worth a `PROPOSED` row.
**Gate:** `pnpm check` green — typecheck 0 errors, **501 passed / 14 skipped**; `pnpm --filter @avd/web build` compiles (the base tsconfig changed, so the app was verified too).

## 2026-07-27 — human sign-off: 10 requirements IN_REVIEW → DONE
**Done:** USER:2026-07-27 "Let's approve all requirements in review state?" — the 10 IN_REVIEW rows in this ledger are approved and moved to DONE. Covers the unified prompt pipeline (REQ-GEN-032), provider routing, the stale-run reaper and Lyria music. Status updated in all three places per `CLAUDE.md` §1.8 (dashboard row · detail block · `Totals:`); `PROGRESS.md` regenerated from the ledgers and independently agrees (129 DONE · 0 IN_REVIEW across all contexts).
...
## 2026-07-27 — REQ-GEN-034 stuck work recovers; anything in flight can be cancelled (→ IN_REVIEW)
**Done:** USER: "2 (video) and 3 (image) are stuck… how to restart?" Diagnosed before answering: a take `running` 7 minutes (under the 30-minute sweep window) and a frame **`queued`** 7 minutes. Two holes, one of them a hole in my own reasoning.
**Decisions:** (1) **A queued row is only "waiting" if something will claim it.** REQ-GEN-027 excluded queued rows because "it never started, so it cannot be orphaned" — true in queue mode, false inline, where a generation runs inside the request that created it and nothing consumes the queue. Inline now reaps queued rows past `staleQueuedMinutes` (5); queue mode never does, because pg-boss owns them and reaping would fight the worker. The reversal is recorded in the old test rather than deleted. (2) `cancelled` is deliberately a different error code and message from `orphaned` — one is a choice, the other a crash, and a user reading the message should not have to guess which. (3) `cancelGeneration` returns false when there was nothing to cancel, so the UI can say "already finished" instead of claiming a cancellation that did not happen.
**Discovered:** the real answer to "how to restart" was, until now, *wait thirty minutes*. Nothing in the UI could stop or restart work. The shot now shows what is in flight, how long it has been, warns past three minutes, and offers cancel.
**Deferred:** cancelling marks the ROW; it cannot abort an HTTP request already at the provider, so a late response is discarded rather than stopped. The real fix for the class is `WORKER_MODE=queue` in dev — recorded in ADR-002 consequences. This makes inline survivable, not correct.
**Gate:** 8/8 `tests/stuck-recovery.int.spec.ts` (red first); 369 passed / 5 skipped across gen+stb+web; tsc clean. Live on the user's project: the sweep recovered the orphaned queued frame, and cancelling the stuck take through the UI produced "take failed · cancelled" with the Take button live again — nothing left in flight.


## 2026-07-27 — process: ADRs, an architecture gate, and a review cadence
**Done:** Three additions the USER asked for, each grounded in something that actually went wrong. (1) **`docs/adr/`** — real records with context, alternatives and consequences, replacing the one-line table in `docs/82-tech-stack.md` §3 that WAS the record until today. Backfilled ADR-001…008 (marked as reconstructed) and wrote the four decisions made since that had no record at all: config-as-only-source (009), one visual prompt pipeline (010, superseding half the v3 rule), reference names never reaching a model (011), consistency by reference not description (012). (2) **`CLAUDE.md` §2B Architecture Baseline** — a gate before the first slice: context map, pipeline shapes, seams, artifact inventory, per-layer test harnesses, ADRs, one thin spike. (3) **`CLAUDE.md` §13 Review & Audit** — four audit types, trigger-based rather than calendar-based, with the measurement commands and the rule that a review producing only a document changes nothing. Plus Prompt 6 and Prompt 7 in `prompts.md`.
**Decisions:** (1) ADRs are IMMUTABLE and superseded rather than edited — the reversal trail is the value, and the v3 reversal is exactly the case that was invisible. (2) Alternatives and costs are mandatory: an ADR listing only benefits is marketing, and the "hard" half is what pays off later. (3) Review triggers are conditions, not a calendar — in a repo moving this fast a monthly review is either late or empty. The strongest trigger is "a user reported something the suite was green for", which is how the first review happened. (4) Escape rate — user-found ÷ test-found defects — is named as the metric that matters.
**Discovered:** the reaper/sweep interference recurred a THIRD time, so I stopped calling it a flake and fixed the design per the new §13 rule: `reapStale` is now one implementation with an optional project scope, both public names delegate, production still reaps unscoped at claim time, and the specs scope to their own fixtures. Three specs had been failing on each other's rows.
**Follow-ups:** REQ-GEN-033 and REQ-STB-059/060/061 remain PROPOSED.
**Gate:** documentation, ADRs and one test-isolation fix. Pure + artifact layers: 182/182 green. Full suite unreliable at the machine's current load average of 186–194 — 7 failures, all integration, all passing when re-run in a targeted group; reported as environmental per `CLAUDE.md` §11, not as a pass.
