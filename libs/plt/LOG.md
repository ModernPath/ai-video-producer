...
**Follow-ups:** human sign-off on the three IN_REVIEW rows · backup + alerting backlog rows · **the Google OAuth client still needs `https://video-creator.modernpath.ai/api/auth/callback/google` added to its authorized redirect URIs** — until it is, nobody can complete a sign-in.
...
- `/api/auth/providers` reports the callback as `https://video-creator.modernpath.ai/api/auth/callback/google`. Clicking through to Google returns `Error 400: redirect_uri_mismatch` — i.e. the request is well-formed and correctly addressed; the URI is simply not yet registered on the client. That is the one open item.
